...
module.exports = {
    ...
    async count(_, response) {
        const strsql = `select count(*) as total from COBERTURA where (deletado = 0 or deletado is null)`;
    ...
    async destroy(request, response) {
        ...
        const strsql = `update COBERTURA set deletado = 1 where cobertura_id = ${cobertura_id}`;
    ...
    async listaUm(request, response) {
        ...
        const strsql = `select 
                ...
                COBERTURA.deletado
            from COBERTURA
            where (COBERTURA.deletado = 0 or COBERTURA.deletado is null) and cobertura_id = ${cobertura_id}`;
        const resultado = await executeQuery(strsql);
        response.status(200).send(resultado);
    },
    ...
    async listaTodos(_, response) {
        const strsql = `select 
                ...
                COBERTURA.deletado
            from COBERTURA
            where (COBERTURA.deletado = 0 or COBERTURA.deletado is null)`;
        const resultado = await executeQuery(strsql);
        response.status(200).send(resultado);
    },
