...
module.exports = {
    ...
    async count(_, response) {
        const strsql = `select count(*) as total from EMISSAO_DOCUMENTO where (deletado = 0 or deletado is null)`;
    ...
    async destroy(request, response) {
        ...
        const strsql = `update EMISSAO_DOCUMENTO set deletado = 1 where documento_id = ${documento_id}`;
    ...
    async listaUm(request, response) {
        ...
        const strsql = `select 
            ...
            where (EMISSAO_DOCUMENTO.deletado = 0 or EMISSAO_DOCUMENTO.deletado is null) and documento_id = ${documento_id}`;
        const resultado = await executeQuery(strsql);
        response.status(200).send(resultado);
    },
    ...
    async listaTodos(_, response) {
        const strsql = `select 
                ...
                EMISSAO_DOCUMENTO.deletado
            from EMISSAO_DOCUMENTO
            where (EMISSAO_DOCUMENTO.deletado = 0 or EMISSAO_DOCUMENTO.deletado is null)`;
        const resultado = await executeQuery(strsql);
        response.status(200).send(resultado);
    },
