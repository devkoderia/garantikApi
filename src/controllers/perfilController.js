...
module.exports = {
    ...
    async count(_, response) {
        const strsql = `select count(*) as total from PERFIL where (deletado = 0 or deletado is null)`;
    ...
    async destroy(request, response) {
        ...
        const strsql = `update PERFIL set deletado = 1 where perfil_id = ${perfil_id}`;
    ...
    async listaUm(request, response) {
        ...
        const strsql = `select 
            ...
            PERFIL.ad_usr
            from PERFIL
            where (PERFIL.deletado = 0 or PERFIL.deletado is null) and perfil_id = ${perfil_id}`;
        const resultado = await executeQuery(strsql);
        response.status(200).send(resultado);
    },
    ...
    async listaTodos(request, response) {
        ...
        const strsql = `select 
            ...
            PERFIL.descricao
            from PERFIL
            where (PERFIL.deletado = 0 or PERFIL.deletado is null) and cliente_id = ${cliente_id}`;
        const resultado = await executeQuery(strsql);
        response.status(200).send(resultado);
    },
